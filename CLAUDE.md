...
In `ci.yml` both `tectonic-docs` and `uv-docs` run (the latter with `artifact-suffix: "uv"` so their artifacts do not collide), but only `tectonic-docs` is a dependency of the `deploy` and `release` jobs — the published PDF is the Tectonic one. Keep both working when touching either.
